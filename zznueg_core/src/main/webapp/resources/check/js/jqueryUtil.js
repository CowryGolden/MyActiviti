(function($) {
	 ...
	 * 定义一个全局的变量，然后的ready方法内部调用本方法，获得basePath
	 */
	 function basePath() {
    ...
    /** 
     * 功能描述：切换皮肤-更换EasyUI主题的方法 
     * @param selectId-主题ID
     * @param cookiesColor-已存在与cookies中，正在使用的主题ID 
     * 
     */
	...
	$("iframe").each(function() {
		//alert("ifr=" + JSON.stringify(this));  //结果：ifr={}，空对象
		//alert("ifr==null-->>" + (JSON.stringify(this)=='{}'));  //结果返回true
...
/** 另一种for循环方法	
    ...
    if ($iframe.length > 0) {    	
        for ( var i = 0; i < $iframe.length; i++) {  
            var ifr = $iframe[i];
            //var dc = ifr.contentWindow.document;
            //alert("ifr=" + JSON.stringify(ifr));  //结果：ifr={}，空对象
            //alert("ifr==null-->>" + (JSON.stringify(ifr)=='{}'));  //结果返回true
        }  
    }*/
	function docchgskin(dc, selectId, cookiesColor) {
		var oldUrl,newUrl;
		if(cookiesColor == 'default' || cookiesColor == '' || cookiesColor == null || cookiesColor == 'null') {
			oldUrl = basePath() + "resources/public/css/easyui.css";
		} else {
			oldUrl = basePath() + "resources/themes/" + cookiesColor + "/easyui.css";
		}
		if(selectId == basePath() + 'default' || selectId == '' || selectId == null || selectId == 'null') {
			newUrl = basePath() + "resources/public/css/easyui.css";
		} else {
			newUrl = basePath() + "resources/themes/" + selectId + "/easyui.css";
		}
	    //removejscssfile(dc, "themes/" + cookiesColor + "/easyui.css", "css");
	    //createLink(dc, "themes/" + selectId + "/easyui.css");
		removejscssfile(dc, oldUrl, "css");
	    createLink(dc, newUrl);
	}
